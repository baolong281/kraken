...
void Feed::start() {
        ...
        session_ = std::make_unique<Poco::Net::HTTPSClientSession>(config.host, config.port, context_);

        Poco::Net::HTTPRequest request(Poco::Net::HTTPRequest::HTTP_GET, this->config.path);
        Poco::Net::HTTPResponse response;
        
        socket_ = std::make_unique<Poco::Net::WebSocket>(*session_, request, response);
        std::cout << "Connected! Status: " << response.getStatus() << " "
        ...
        socket_->sendFrame(msg.data(), msg.size(), Poco::Net::WebSocket::FRAME_TEXT);
        std::cout << "sending: " << msg << std::endl;
        
        char buffer[1024];
        int flags;
        int n = socket_->receiveFrame(buffer, sizeof(buffer), flags);
        std::cout << "Successfully subscribed to feed: " << std::string(buffer, n) << std::endl;
        
        running_.store(true, std::memory_order_release);
        worker_ = std::thread(&Feed::run, this);
    } catch (const std::exception& e) {
        std::cerr << "Failed to start feed: " << e.what() << std::endl;
